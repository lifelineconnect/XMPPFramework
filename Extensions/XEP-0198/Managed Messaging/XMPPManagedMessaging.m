...
@interface XMPPManagedMessaging () <XMPPStreamManagementDelegate>

@property (nonatomic, strong, readonly) id<XMPPManagedMessagingStorage> storage;

@end

@implementation XMPPManagedMessaging

- (instancetype)initWithStorage:(id<XMPPManagedMessagingStorage>)storage dispatchQueue:(dispatch_queue_t)dispatchQueue
{
...
- (void)xmppStream:(XMPPStream *)sender didSendMessage:(XMPPMessage *)message inContextOfEvent:(XMPPElementEvent *)event
{
    ...
    [self.storage registerOutgoingManagedMessageID:[message elementID] withEvent:event];
...
- (void)processStreamManagementAcknowledgementForStanzaIDs:(NSArray *)stanzaIDs
{
    ...
    [self.storage registerManagedMessageConfirmationForSentMessageIDs:managedMessageIDs];
