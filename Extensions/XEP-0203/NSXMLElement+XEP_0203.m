...
- (BOOL)wasDelayed
{
    return [self anyDelayChildElement] != nil;
}
...
- (NSDate *)delayedDeliveryDate
{
	...
	NSXMLElement *delay = [self delayChildElement];
	if (delay)
	{
	...
	NSXMLElement *legacyDelay = [self legacyDelayChildElement];
	if (legacyDelay)
	{
		NSDate *stamp;
...
{
    NSString *delayOriginJIDString = [[self anyDelayChildElement] attributeStringValueForName:@"from"];
    return delayOriginJIDString ? [XMPPJID jidWithString:delayOriginJIDString] : nil;
}

- (NSString *)delayReasonDescription
{
    return [self anyDelayChildElement].stringValue;
}

- (NSXMLElement *)delayChildElement
{
    return [self elementForName:@"delay" xmlns:@"urn:xmpp:delay"];
}

- (NSXMLElement *)legacyDelayChildElement
{
    return [self elementForName:@"x" xmlns:@"jabber:x:delay"];
}

- (NSXMLElement *)anyDelayChildElement
{
    return [self delayChildElement] ?: [self legacyDelayChildElement];
}
